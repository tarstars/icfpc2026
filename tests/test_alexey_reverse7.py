"""reverse_07: 13x13 re-pack of reverse_06 -- same program, smaller box."""
...
from littleman.alexey_reverse7 import build_reverse7
...
def test_submitted_file_matches_generator():
    stored = (ROOT / "submissions/reverse-a-list/reverse_07.man").read_text()
    assert stored == TEXT
...
def test_geometry():
    ...
    ring = sorted((len(p.cells) for p in machine.pipes), reverse=True)[:2]
    assert sum(ring) >= 19, "ring capacity vs a measured peak occupancy of 16"
    assert not find_shared_walls(TEXT)
    lines = TEXT.split("\n")
    assert max(len(line) for line in lines) == 13
    assert len([line for line in lines if line.strip()]) == 13
...
def test_pipe_resolution():
    ...
    ring_in, output = (0, 7), (10, 10)
    ring_out, input_in = (8, 3), (8, 1)  # both land on the relay's floor
    assert resolved[(5, 6)] == ring_out, "the loop's s must feed the ring"
    for cell in [(4, 9), (5, 11), (7, 10)]:
        assert resolved[cell] == output, f"{cell} must print"
    for cell in [(2, 9), (4, 8), (5, 7), (7, 9)]:
        assert resolved[cell] == ring_in, f"{cell} must read the ring"
    # relay: input sorts first, so `R` drains the input frame before relays
    assert resolved[(5, 1)] == input_in


def test_public_cases():
    report = judge_problem(TEXT, PROBLEM)
    assert report.cases_passed == report.cases_total == 8
    assert report.footprint == 169
    assert report.score < 62_769.0, "must beat reverse_06"
