"""Physical gates for replacing one selected indexed pipe record."""
...
from littleman.llm_selectedreplace import (
...
class Script:
    def __init__(self, requests):
        ...
        self.expected = [
            item for request in requests for item in selectedreplace_reference(request)
        ]
        self.output = []
...
def text():
    return build_selectedreplace_rig()


def test_generator_is_deterministic_and_server_safe(text):
    assert build_selectedreplace_rig() == text
...
def test_physical_inactive_send_receive_and_second_slot(text):
    ...
    requests = [selectedapply_reference(request) for request in copied]
