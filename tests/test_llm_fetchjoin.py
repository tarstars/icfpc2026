"""Physical parity for one runtime FETCH-join state-map cycle."""
...
from littleman.llm_fetchjoin import (
    build_fetchjoin_rig,
    fetchjoin_reference,
)
...
class Script:
    def __init__(self, stream):
        ...
        self.expected = fetchjoin_reference(stream)
        self.output = []
...
def text():
    return build_fetchjoin_rig()


def test_generator_is_deterministic_and_server_safe(text):
    assert build_fetchjoin_rig() == text
