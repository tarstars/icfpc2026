...
from littleman.llm_indexcopy import (
    build_indexcopy_rig,
...
class Script:
    def __init__(self, tokens, copies=2):
        self.input = list(tokens)
        self.expected = indexmulticopy_reference(tokens, copies)
        self.output = []
...
def test_physical_three_copies(triple_text):
    state = indexed_state(CASES[5])
    script = Script(state, 3)
    server_compat.validate_layout(triple_text)
    alexey_pipecheck.check(triple_text)
    result = Machine.parse(triple_text).run(max_ticks=10_000_000, controller=script)
    assert result.error is None
    assert result.status == "passed"
    assert script.output == script.expected
