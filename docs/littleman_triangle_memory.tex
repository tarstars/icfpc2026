...
about 240 --- is its smaller sibling \texttt{triangle\_03.man}
...
footprint, and it ends by deliberately sacrificing its man.
...
\subsection{The record holder: one room, no halt, a sacrificed man}
\label{sec:tri3}

\texttt{triangle\_03.man} beats the machine above $891\to 832$ while
taking \emph{more} ticks (13 vs 11) --- because the score squares the
bounding box: $64\times13=832 < 81\times11=891$. Losing one column and one
row was worth two extra ticks. Its other trick is more radical: it contains
\textbf{no \op{H} at all}.
...
The block-graph ends in \texttt{(goto \_\_wall) (wall)}: after \op{s},
the man walks over a blank cell and \emph{into the west wall}. In the raw
simulator that run's status is literally \texttt{error} --- and the case
still passes, because the answer reaches \texttt{O} on the very tick of the
fatal step, and the server accepts a run whose final tick both delivers the
last output and steps into a wall (a behaviour we probed and encoded in
\texttt{server\_compat} as final-wall-step semantics). The \op{H} cell,
and the room width to reach it, are simply deleted.

Trace for $n=3$ (single man; same column conventions as above):
...
Ticks 1--5 are the same $n^2+n$ prologue as before, ending with \op{v}
into row 2, whose lone \op{M} parks $n^2+n$ in \op{B} in passing; row 3
walked westward reads \op{<\,1\,W\,\}\,s}: load 1, swap, halve, send.
Ticks 12--13: the man crosses the blank cell and dies on the wall exactly
as 6 arrives at \texttt{O}. Three lessons: \emph{the footprint is squared,
so trade ticks for cells; a man you no longer need costs nothing to kill;
and the scoring clock, not the man's fate, decides the run} --- but note
the sibling \texttt{triangle\_04} fails 0/6 from getting exactly this
timing wrong, so the sacrifice must be \emph{proven}, not eyeballed.
...
team's best live Memory score is a sibling, hand-tuned lineage ---
\texttt{alexey-memory\_06}, $34\times32$ --- built on the same ideas.)
