...
\texttt{submissions/gradebook/gradebook\_05.man}: $382\times307$, footprint
$382^2=145\,924$, 16 rooms, 30 pipes, and 14 men. Its preserved live result is
20/20 at score $47\,115\,780\,604$. The state machines originate in
\texttt{src/littleman/gradebook.py}; later variants first tightened their
layout, then squeezed and staircase-folded the same architecture.
\texttt{gradebook\_05} additionally replaces three fixed worker delays with
blocking ring receives and keeps only whole-machine-validated folds. The last
...
$379\times5$ room contains only nine interior operations. A compact arbiter
or two-level merge tree is promising, although shrinking it alone does not
beat the current 382-column bound.
...
The score consequence matters. The current machine is width-bound at 382:
the front-end room is 381 cells wide, while the external pipe clearance
reaches the final column. More row folding is ``banked'' but does not reduce the squared
...
arrangement was worse with the present $93$-column engine rooms; it should be
...
\texttt{submissions/gradebook/gradebook\_05.man} & the live Grade Book artifact catalogued here\\
...
\texttt{src/littleman/gradebook\_components.py} & Grade Book room contracts and live optimized generator\\
...
the checked-in \texttt{gradebook\_05.man}. Live scores come only from
