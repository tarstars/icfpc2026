...
| Exact Rust executor | `src/littleman/rustexec.py`, `rust/` | integrator | `verified`, peer-approved; pending `main` promotion | accelerated differential tests and independent simulation batches after integration | Versioned dense IR; PyO3 and standalone Rayon CLI; 2,012-test LLM suite passed in 53.08 s versus 858.86 s under Python; whole-state corpus, mutation, Split, cache, and 1/N-worker parity tests pass. Missing PyO3 falls back to `fastsim` (108 passed, 11 native-only skips). Python remains the parser. |
...
`LITTLEMAN_RUSTEXEC=0` forces the clean-checkout fallback for validation.
Ordinary `rustexec.Machine` and cached case batches then use `fastsim`;
compressed-IR encoding and official Split execution require the native
extension and fail or skip with an explicit reason.
